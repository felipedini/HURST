...
# Função para obter os dados de preço
def obter_dados_preco(symbol, timeframe, n_bars):
    try:
        ...
        if rates is None:
            ...
            return None
        prices = [rate[4] for rate in rates]  # Obtemos apenas os preços de fechamento (close)
        return prices
    except Exception as e:
        print(f"Erro ao obter dados de {symbol}: {e}")
        return None

# Define os ativos que você deseja plotar
...
for i, asset in enumerate(assets):
    ...
    if prices is not None:
        try:
            ...
            # Plota os dados no subplot atual
            axs[i // 6, i % 6].plot(range(len(prices)), prices)
            axs[i // 6, i % 6].set_title(f"{asset}\nTimeframe: {mt5.TIMEFRAME_D1}\nHurst: {H:.2f}")

            # Define o intervalo do eixo y de forma automática
